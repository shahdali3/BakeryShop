...
class ShiftService {
  ...
  async startShift(data: IStartShift) {
    try {
      ...
      const isChashierHaveAnotherShiftOpen = await this.shiftDataSource.findOne(
        { cashierId, isCancelled: false }
      );
      if (isChashierHaveAnotherShiftOpen) {
        throw new ApiError(
          "You already have running shift, cannot create another",
          400
        );
      }
  ...
  async endShift(data: IEndShift) {
    try {
      ...
      const isChashierHaveShiftOpen = (await this.shiftDataSource.findOne({
        cashierId,
        isCancelled: false,
      })) as IShift;
      ...
      if (areThereNotPaidOrders.length > 0) {
        throw new ApiError(
          "Must all orders are paid or can not end this shift",
          500
        );
      }
